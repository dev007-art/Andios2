...
public class AddStockFrame extends JFrame {
	...
	private JFormattedTextField txtConsignmentDate;
	...
	public AddStockFrame() {
		...
		txtConsignmentDate = new JFormattedTextField(df);
		txtConsignmentDate.setColumns(10);
		txtConsignmentDate.setBounds(123, 58, 168, 24);
		txtConsignmentDate.setValue(new Date());
		...
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					p.setOid(orderId);
					p.setsName(sName);
					Client.purchaseDao.add(p, Session.getToken());
					MyToolKit.showMessage("添加采购订单成功！");
					
					observable.setChanged();
					observable.notifyObservers();
					
					cbSName.setSelectedItem(null);
					txtOrderId.setText("");
					cbBaleName.setSelectedItem(null);
					txtMoney.setText("");
					txtCount.setText("");
					txtConsignmentDate.setValue(new Date());
		...
		btnAdd.setBounds(178, 156, 113, 27);
